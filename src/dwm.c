...
movemouse(const Arg *arg)
{
	...
	do {
		...
		case MapRequest:
			...
			break;
		case MotionNotify:
			if ((ev.xmotion.time - lasttime) <= (1000 / fps))
				continue;
...
void layoutscroll(const Arg *arg) {
	if (!arg || !arg->i)
		return;
	int switchto = selmon->currlt + arg->i;
...
tile(Monitor *m)
{
	unsigned int i, n, h, r, oe = enablegaps, ie = enablegaps, mw, my, ty; // mod
    Client *c;
    ...
    } else { mw = m->ww - 2*m->gappov*oe + m->gappiv*ie; } // mod
		
	for (i = 0, my = ty = m->gappoh*oe, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) { // mod
		if (i < m->nmaster) {
            r = MIN(n, m->nmaster) - i;
